%%%% Runs energetics model

 function out = energetics_model(TAN, CRtot, TEP, Ox_capacity, x_ATPase)
Cr = CRtot*0.7; % M/L cell
PCr = CRtot*0.3; % M/L cell
...
ATP_c = (TAN-Vmito*Wmito_x*Atot)/(Vcyto*Wcyto+Vmito*Wmito_i);
AMP_c = 1e-9;
ADP_c = 1e-9;
...
%% Results
ATP_cyto = x(end,8); % Cytosolic total ATP concentration (M·(L cytosol water)^-1)
ADP_cyto = x(end,9); % Cytosolic total ADP concentration (M·(L cytosol water)^-1)
AMP_cyto = x(end,13); % Cytosolic total AMP concentration (M·(L cytosol water)^-1)
Pi_cyto= x(end,10); % Cytosolic total Pi concentration (M·(L cytosol water)^-1)
ATP_im = x(end,18); % im total ATP concentration (M·(L cytosol water)^-1)
ADP_im = x(end,17); % im total ADP concentration (M·(L cytosol water)^-1)
AMP_im = x(end,19); % im total AMP concentration (M·(L cytosol water)^-1)
Pi_im= x(end,16); % im total Pi concentration (M·(L cytosol water)^-1)
ATP_matrix = x(end,4); % Matrix total ATP concentration (M·(L cytosol water)^-1)
ADP_matrix = x(end,3); % Matrix total ADP concentration (M·(L cytosol water)^-1)
Pi_matrix= x(end,5); % Matrix total Pi concentration (M·(L cytosol water)^-1)
PCr_cyto = x(end,11); % Cytosolic total phosphocreatine concentration (M·(L cytosol water)^-1)
Cr_cyto = x(end,12); % Cytosolic total creatine concentration (M·(L cytosol water)^-1)

% PCr/ATP
PCr_cell = Vcyto*Wcyto*PCr_cyto; % Cellular phosphocreatine
Cr_cell = Vcyto*Wcyto*Cr_cyto; % Cellular creatine
ATP_cell = Vcyto*Wcyto*ATP_cyto+Vmito*Wmito_x*ATP_matrix+Vmito*Wmito_i*ATP_im; % Cellular ATP
PCrATP = PCr_cell/ATP_cell; % Creatine phosphate ATP ratio, unitless

% Fluxes
J(3)=J(3);
lcell = 1e-3*Rc_t/rho; % L cell/g tissue; Vinnakota etal Am J Physiol Heart Circ Physiol. 2004 May;286(5):H1742-9
J_ATPase_cell = J(3)*Vcyto; % J_ATPASE_cytoplasm % J_ATPase: M/s/(l cell)

J_ATPase_tissue = J_ATPase_cell*lcell; % M/s/g tissue

J_C4 = J(9); % J_ETC4_im_to_matrix
J_ANT = J(12);
J_HLEAK = J(10);
MVO2_tissue = (J_C4/2)*Vmito*Rc_t/1000/rho*60*1e6; % Oxygen consumption rate (uM·min^-1·(g tissue)^-1)

MgATP_cytoplasm = y(1); % Cytosolic MgATP concentration (M·(L cytosol water)^-1)
MgADP_cytoplasm = y(2); % Cytosolic MgADP concentration (M·(L cytosol water)^-1)
fPi_cytoplasm = y(3); % Cytosolic unchelated Pi concentration (M·(L cytosol water)^-1)
dGrATPase = y(4); % ATP hydrolysis free energy, kJ/mol
